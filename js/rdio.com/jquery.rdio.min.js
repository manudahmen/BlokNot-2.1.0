(function () {
     ...
     */
    (function (c) {
        c.fn.rdio = function b(d) {
            var e = c(this);
            var l = e.data("rdio");
            if (l != undefined) {
                return l
            }
            var f = "rdio_swf_" + Math.floor(Math.random() * 10000);
            c("<div></div>").attr("id", f).prependTo(e);
            var m = f + "_cb";
            var h = {};
            h.ready = function (n) {
                e.trigger("ready.rdio", [n])
            };
            h.playStateChanged = function (n) {
                e.trigger("playStateChanged.rdio", [n])
            };
            h.playingTrackChanged = function (o, n) {
                e.trigger("playingTrackChanged.rdio", [o, n])
            };
            h.playingSourceChanged = function (n) {
                e.trigger("playingSourceChanged.rdio", [n])
            };
            h.volumeChanged = function (n) {
                e.trigger("volumeChanged.rdio", [n])
            };
            h.muteChanged = function (n) {
                e.trigger("muteChanged.rdio", [n])
            };
            h.positionChanged = function (n) {
                e.trigger("positionChanged.rdio", [n])
            };
            h.queueChanged = function (n) {
                e.trigger("queueChanged.rdio", [n])
            };
            h.shuffleChanged = function (n) {
                e.trigger("shuffleChanged.rdio", [n])
            };
            h.repeatChanged = function (n) {
                e.trigger("repeatChanged.rdio", [n])
            };
            h.updateFrequencyData = function (n) {
                e.trigger("updateFrequencyData.rdio", [n])
            };
            h.playingSomewhereElse = function () {
                e.trigger("playingSomewhereElse.rdio")
            };
            h.freeRemainingChanged = function (n) {
                e.trigger("freeRemainingChanged.rdio", [frequencyData])
            };
            window[m] = h;
            var i = {playbackToken: d, domain: document.domain, listener: m};
            var j = {allowScriptAccess: "always"};
            var k = {};
            var g = {embed: null};
            a.embedSWF("http://www.rdio.com/api/swf/", f, 1, 1, "9.0.0", "", i, j, k, function (n) {
                g.embed = c("#" + f).get(0)
            });
            g.play = function (o, n) {
                this.embed.rdio_play(o, n)
            };
            g.pause = function () {
            ...
            g.next = function (n) {
                this.embed.rdio_next(n)
            };
            g.previous = function () {
                this.embed.rdio_previous()
            };
            g.seek = function (n) {
                this.embed.rdio_seek(n)
            };
            g.setShuffle = function (n) {
                this.embed.rdio_setShuffle(n)
            };
            g.setRepeat = function (n) {
                this.embed.rdio_setRepeat(n)
            };
            g.queue = function (o, n) {
                this.embed.rdio_queue(o, n)
            };
            g.setVolume = function (n) {
                this.embed.rdio_setVolume(n)
            };
            g.setMute = function (n) {
                this.embed.rdio_setMute(n)
            };
            g.playQueuedTrack = function (n, o) {
                this.embed.rdio_playQueuedTrack(n, o)
            };
            g.moveQueuedSource = function (o, n) {
                this.embed.rdio_moveQueuedSource(o, n)
            };
            g.clearQueue = function () {
                this.embed.rdio_clearQueue()
            };
            g.setCurrentPosition = function (n) {
                this.embed.rdio_setCurrentPosition(n)
            };
            g.removeFromQueue = function (n) {
                this.embed.rdio_removeFromQueue(n)
            };
            g.sendState = function () {
                this.embed.rdio_sendState()
            };
            g.startFrequencyAnalyzer = function (n) {
                this.embed.rdio_startFrequencyAnalyzer(n)
            };
            g.stopFrequencyAnalyzer = function () {
