...
<form action="page.xhtml.php" method="GET">
    ...
    <textarea rows="24" cols="80"
              name="contenu"><?php echo file_get_contents($dataDir . $pathSep . $emplacement . $document . ".txt");
    ...
    <select name="classeur">
        ...
        while (($f = readdir($fh)) != NULL) {
            ...
            if (strlen($f) > 5 && substr($f, 0, 5) == "CLASS") {
                ?>
                <option name="<?php echo substr($f, 5); ?>"
                        value="<?php echo substr($f, 5); ?>"><?php echo substr($f, 5); ?></option>
                <?php
            }
        }
        ?>
