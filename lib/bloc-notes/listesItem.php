...
function listerImage($action = "list-view", $classeur = ".")
{
    ...
    while (($f = readdir($dirh)) != NULL) {
        ...
        if (strtolower(substr($f, -4)) == ".png"
            ...
            ?>
            <a class='miniImg' href="<?= $actionurl ?>"><img src='<?= "$userdataurl/./$f" ?>'
                                                             class="miniImg"><?php echo $classeur . "/" . $f; ?></a>
            <?php
...
function listerTexte($action = "list", $classeur = ".")
{
    ...
    while (($f = readdir($dirh)) != NULL) {
        ...
        if (strtolower(substr($f, -4)) == ".txt") {
            ?>
            <a class='miniImg' href="<?= $urlaction ?>"><img src='<?= "$urlApp/composant/reader.txt/txt.jpg" ?>'
                                                             class='miniImg'
                                                             alt='Test Type File'/><?php echo $classeur . "/" . $f; ?>
            </a>
            <?php
...
function listerBN()
{
    ...
    if ($result != NULL) {
        ...
        $value = $result[$i];
        print_r($value);
        while ($value) //foreach($item as $no => $value)
        {
