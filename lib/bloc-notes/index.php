...
<form name="blocnoteForm" method="POST">
    ...
    <?php if ($Tables->findType($document) == "text") { ?>
        ...
        <div id="HtmlView"></div><?php } ?>
    <?php if ($Tables->findType($document) == "image") { ?>
        ---- <img src="<?php echo $userdataurl . $pathSep . $document; ?>"/><a
            href="editimage.php?imagepng=<?= $document ?>">Editer image</a><a
            href="publish.php?filename=<?= $document ?>">Publier</a>
    <?php } ?>
    <p>-</p>

    <p>T&eacute;l&eacute;charger: <a
            href="<?php echo $urlbase . "/data/" . $fgmembersite->UserFullName() . "/" . $document; ?>"><?php echo $urlbase . "/data/" . $document; ?></a>
    </p>

    <p>T&eacute;l&eacute;charger URL: <input type"text" name="download" value="" size="20" /><input type="submit"
                                                                                                    name="cmd"
                                                                                                    value="<?php echo $trad['CMD']['DOWNLOAD']; ?>"/>
    </p>

    <p>
        <button name="" onclick="javascript:inserer();">Insérer</button>
    </p>
</form>
<p>
    <button name="" onclick="javascript:htmlView();">As HTML</button>
</p>
