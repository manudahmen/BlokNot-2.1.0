...
class GracenoteWebAPI
{
    ...
    public function register($clientID = null)
    {
        ...
        if ($clientID === null) {
            $clientID = $this->_clientID . "-" . $this->_clientTag;
        }
    ...
    protected function _checkResponse($response = null)
    {
        ...
        switch ($status) {
            case "ERROR":
                throw new GNException(GNError::API_RESPONSE_ERROR, (string)$xml->MESSAGE);
                break;
            case "NO_MATCH":
                throw new GNException(GNError::API_NO_MATCH);
                break;
            default:
                if ($status != "OK") {
    ...
    protected function _parseResponse($response)
    {
        ...
        foreach ($xml->RESPONSE->ALBUM as $a) {
            ...
            foreach ($a->TRACK as $t) {
                ...
                if (isset($t->GENRE)) {
                    $obj["genre"] = $this->_getOETElem($t->GENRE);
                }
                if (isset($t->ARTIST_ERA)) {
                    $obj["artist_era"] = $this->_getOETElem($t->ARTIST_ERA);
                }
                if (isset($t->ARTIST_TYPE)) {
                    $obj["artist_type"] = $this->_getOETElem($t->ARTIST_TYPE);
                }
                if (isset($t->ARTIST_ORIGIN)) {
